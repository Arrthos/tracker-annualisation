...
sol_date = st.sidebar.date_input("Journée de Solidarité", value=date(2026, 6, 1)) # Pentecôte 2026 par défaut
...
st.markdown(f"""
    ...
    <div style="display:flex; justify-content:space-around; margin-bottom:20px; background:rgba(255,255,255,0.02); padding:10px; border-radius:10px; border:1px solid rgba(255,255,255,0.1);">
...
with tab1:
    ...
    def frag_heures():
        with st.expander("➕ Enregistrer des heures", expanded=False):
            with st.form("h_form_new", clear_on_submit=True):
...
with tab2:
    ...
    def frag_conges():
        with st.expander("➕ Poser un congé", expanded=False):
            with st.form("c_form_new", clear_on_submit=True):
